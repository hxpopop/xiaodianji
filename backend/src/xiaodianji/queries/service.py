...
CENT = Decimal("0.01")
ANOMALY_MESSAGES = {"amount_mismatch": "金额不一致", "duplicate_idempotency": "重复提交异常"}
...
class QueryService:
    ...
    def __init__(self, session_factory: async_sessionmaker[AsyncSession], customer_service: CustomerService, *, now: Callable[[], datetime] | None = None) -> None:
        self.session_factory, self.customer_service = session_factory, customer_service
    ...
    async def query(self, shop_id: UUID, question: str) -> QueryResponse:
        ...
        try:
            handlers = {QueryIntent.CUSTOMER_BALANCE: lambda: self._balance(shop_id, parsed), QueryIntent.HISTORICAL_QUOTE: lambda: self._quote(shop_id, parsed), QueryIntent.DAILY_FLOW: lambda: self._daily(shop_id), QueryIntent.OVERDUE: lambda: self._overdue(shop_id), QueryIntent.PENDING: lambda: self._pending(shop_id), QueryIntent.ANOMALY: lambda: self._anomaly(shop_id)}
    ...
    async def _customer(self, shop_id: UUID, parsed: ParsedQuery) -> tuple[UUID | None, QueryResponse | None]:
        ...
        candidates = [item.model_dump(mode="json") for item in match.candidates]
        return None, QueryResponse(answer="客户名称存在歧义，请选择客户后再查询。" if candidates else "没有找到该客户，请检查客户名称后再查询。", ambiguity={"candidates": candidates})
    ...
    async def _balance(self, shop_id: UUID, parsed: ParsedQuery) -> QueryResponse:
        ...
        details: list[dict[str, Any]] = [{"id": x.id, "type": "transaction", "occurred_at": x.occurred_at, "amount": _money(x.total_amount), "evidence_id": x.source_evidence_id if x.source_evidence_id in owned else None} for x in transactions] + [{"id": x.id, "type": "payment", "paid_at": x.paid_at, "amount": _money(x.amount), "evidence_id": x.source_evidence_id if x.source_evidence_id in owned else None} for x in payments]
    ...
    async def _quote(self, shop_id: UUID, parsed: ParsedQuery) -> QueryResponse:
        ...
        assert customer_id
        async with self.session_factory() as session:
            statement = select(Quote, QuoteItem).join(QuoteItem).where(Quote.shop_id == shop_id, Quote.customer_id == customer_id).order_by(Quote.quoted_at.desc(), Quote.id, QuoteItem.id)
            if parsed.product:
        ...
        details = [{"id": x.id, "quote_id": quote.id, "product": x.product, "quantity": str(x.quantity), "unit": x.unit, "unit_price": _money(x.unit_price), "subtotal": _money(x.subtotal), "quoted_at": quote.quoted_at, "evidence_id": evidence_id} for x in items]
    ...
    async def _daily(self, shop_id: UUID) -> QueryResponse:
        async with self.session_factory() as session:
            ...
            try: local_now = self.now().astimezone(ZoneInfo(timezone_name or "Asia/Shanghai"))
            except (TypeError, ValueError, ZoneInfoNotFoundError): local_now = self.now().astimezone(ZoneInfo("Asia/Shanghai"))
            start = local_now.replace(hour=0, minute=0, second=0, microsecond=0); end = start + timedelta(days=1)
    ...
    async def _overdue(self, shop_id: UUID) -> QueryResponse:
        ...
        details = [{"id": x.id, "customer_name": name, "due_at": x.due_at} for x, name in rows]
    ...
    async def _anomaly(self, shop_id: UUID) -> QueryResponse:
        ...
        details = [{"id": x.id, "type": x.type, "severity": x.severity, "message": ANOMALY_MESSAGES.get(x.type, "账目异常"), "created_at": x.created_at} for x in records]
