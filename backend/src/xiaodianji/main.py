...
def create_app(*, customer_service: Any | None = None, manual_record_service: Any | None = None, confirmation_service: Any | None = None, ledger_service: Any | None = None, evidence_service: Any | None = None, record_workflow: Any | None = None, max_request_body_bytes: int = DEFAULT_MAX_REQUEST_BODY_BYTES) -> FastAPI:
    workflow = SQLAlchemyLedgerWorkflow(async_session_factory)
    ledger_backend = ledger_service or workflow
    settings = Settings()
    if evidence_service is None:
        storage = Boto3ObjectStorage(endpoint_url=settings.object_storage_endpoint, access_key=settings.object_storage_access_key, secret_key=settings.object_storage_secret_key, bucket=settings.object_storage_bucket)
    ...
    app.state.record_workflow = record_workflow or RecordWorkflow(confirmation_workflow=workflow, extraction_provider=extraction_provider_from(settings), asr_provider=asr_provider_from(settings), evidence_service=evidence_service, customer_service=customer_backend)
