...
export class RecordStore {
  ...
  private requireShop(){if(!this.state.shopId)throw new Error('尚未配置商户，请联系管理员设置商户信息后再记账。')}
  private adopt(draft: ConfirmationRead) { this.state.draft = draft; this.state.error = ''; return draft }
  async createTextDraft(text: string) { this.requireShop(); return this.run(() => this.client.createTextDraft(this.state.shopId, text)) }
  async createManualDraft(draft: RecordDraft) { this.requireShop(); return this.run(() => this.client.createManualDraft(this.state.shopId, draft)) }
  ...
  private async run(action: () => Promise<ConfirmationRead>) { this.state.saving = true; try { return this.adopt(await action()) } catch (error) { this.state.error = error instanceof Error ? error.message : '操作未完成，请稍后重试或改用手动输入。'; throw error } finally { this.state.saving = false } }
}
