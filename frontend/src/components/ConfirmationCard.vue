...
import type { ConfirmationRead, RecordDraft, TransactionDraft } from '../api/types'
...
const isItemRecord = computed(() => draft.value.target_type !== 'payment')
...
function updateItem(index: number, item: TransactionDraft['items'][number]) {
  if (!isItemRecord.value) return
  const items = [...draft.value.items]
  items[index] = { ...item, subtotal: Number((item.quantity * item.unit_price).toFixed(2)) }
  draft.value = { ...draft.value, items, total_amount: Number(items.reduce((sum, line) => sum + line.subtotal, 0).toFixed(2)) } as RecordDraft
}
...
<template>
  <article class="confirmation-card" aria-labelledby="confirmation-heading">
    ...
    <section v-if="isItemRecord" aria-labelledby="items-heading"><h2 id="items-heading">商品明细</h2><LineItemEditor v-for="(item, index) in draft.items" :key="index" :item="item" :index="index" :low-confidence="confidenceBelow(`items.${index}.quantity`)" @update:item="updateItem(index, $event)" /></section>
